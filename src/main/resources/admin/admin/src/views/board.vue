<template>
<div class="content box9" :style='{"alignContent":"flex-start","minHeight":"100vh","flexWrap":"wrap","background":"#180312","display":"flex","width":"100%","position":"relative","height":"auto"}'>
	<!-- 标题 -->
	...
	<div :style='{"alignContent":"flex-start","padding":"20px","margin":"0 3% 20px 1%","flexWrap":"wrap","background":"rgba(255,255,255,.1)","display":"flex","width":"26%","height":"200px"}'>
		...
		<div :style='{"width":"100%","alignItems":"center","background":"rgba(255,255,255,.1)","justifyContent":"center","display":"flex"}'>
			...
			<div :style='{"padding":"0 20px","alignItems":"center","flex":"1","justifyContent":"space-between","display":"flex"}'>
				<div :style='{"lineHeight":"32px","fontSize":"16px","color":"#fff","height":"32px"}'>数据分析总数</div>
				<div :style='{"lineHeight":"32px","fontSize":"20px","color":"#fff","fontWeight":"bold","height":"32px"}'>{{shujufenxiCount}}</div>
			</div>
		</div>
            ...
            <div class="echarts1">
                <div id="shujufenxiChart1" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts2">
                <div id="shujufenxiChart2" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts3">
                <div id="shujufenxiChart3" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts4">
                <div id="shujufenxiChart4" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts5">
                <div id="shujufenxiChart5" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts6">
                <div id="shujufenxiChart6" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts7">
                <div id="shujufenxiChart7" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts8">
                <div id="shujufenxiChart8" style="width:100%;height:100%;"></div>
            </div>
            <div class="echarts9">
                <div id="shujufenxiChart9" style="width:100%;height:100%;"></div>
            </div>
...
export default {
  ...
  mounted(){
    ...
    this.shujufenxiChat6();
    this.shujufenxiChat7();
    this.shujufenxiChat8();
    this.shujufenxiChat9();
  },
  created() {
    this.$nextTick(()=>{
		this.times()
...
//统计接口6
    shujufenxiChat6() {
      this.$nextTick(()=>{
        ...
        var shujufenxiChart6 = echarts.init(document.getElementById("shujufenxiChart6"),'macarons');
        this.$http({
            url: "shujufenxi/group/shifouyinjiu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].shifouyinjiu);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].shifouyinjiu
                    })
                }
                var option = {};
                ...
                // 使用刚指定的配置项和数据显示图表。
                shujufenxiChart6.setOption(option);
				

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shujufenxiChart6.resize();
                };
            }
        });
      })
...
//统计接口7
    shujufenxiChat7() {
      this.$nextTick(()=>{
        ...
        var shujufenxiChart7 = echarts.init(document.getElementById("shujufenxiChart7"),'macarons');
        this.$http({
            url: "shujufenxi/group/shifouyundong",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].shifouyundong);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].shifouyundong
                    })
                }
                var option = {};
                let titleObj = this.pie.title
                ...
                // 使用刚指定的配置项和数据显示图表。
                shujufenxiChart7.setOption(option);
				

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shujufenxiChart7.resize();
                };
            }
        });
      })
...
//统计接口8
    shujufenxiChat8() {
      this.$nextTick(()=>{
        ...
        var shujufenxiChart8 = echarts.init(document.getElementById("shujufenxiChart8"),'macarons');
        this.$http({
            url: "shujufenxi/group/xinzanggongnengshifoulianghao",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].xinzanggongnengshifoulianghao);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].xinzanggongnengshifoulianghao
                    })
                }
                var option = {};
                let titleObj = this.bar.title
				titleObj.text = '心脏功能是否良好'
                ...
                // 使用刚指定的配置项和数据显示图表。
                shujufenxiChart8.setOption(option);
				

                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shujufenxiChart8.resize();
                };
            }
        });
      })
...
//统计接口9
    shujufenxiChat9() {
      this.$nextTick(()=>{
        ...
        var shujufenxiChart9 = echarts.init(document.getElementById("shujufenxiChart9"),'macarons');
        this.$http({
            url: `shujufenxi/valueMul/bianhao?yColumnNameMul=shengao,zhongliang`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                shujufenxiChart9.setOption(option);


                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shujufenxiChart9.resize();
                };
            }
        });
      })
